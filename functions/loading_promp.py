...
def load_prompt(answer_AI, answer_AI_type):
    ...
    if answer_AI_type == "Normal":
        ...
        template = base_prompt + """
        You are a nutritional coach that wants to push the user into healthier foods.
    ...
    if answer_AI == "Long and precise":
        template += """
        The answers you give have to be extensive, in ten to thirty sentences.
        """ 
        template += base_prompt_2

    elif answer_AI == "Summary":
        template += """
        You are giving a summary, in five to fifteen sentences.
